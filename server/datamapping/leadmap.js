...
const updatedLeads = loadJSON('updated_leads.json');
const labelsData = loadJSON('labels.json'); // Load your label data

const findOrCreateLabel = async (labelData) => {
  try {
    ...
    if (!label) {
      label = new Label({
        ...
        pipeline_id: labelData.pipeline_id,
        created_by: labelData.created_by,
        created_at: new Date(labelData.created_at),
        updated_at: new Date(labelData.updated_at),
      });
      await label.save();
    }
    return label._id; // Return the ID of the existing or newly created label
  } catch (error) {
...
const findOrCreateProductStage = async (stageData) => {
  try {
    ...
    // Check if the product stage already exists
    const productStage = await ProductStage.findOne({ 
      name: stageData.name, 
      product_id: productId 
    });

    // If it doesn't exist, create a new product stage
    if (!productStage) {
      const newProductStage = new ProductStage({
        name: stageData.name,
        product_id: productId,
        pipeline_id: stageData.pipeline_id, // Include pipeline_id here
        order: stageData.order,
        created_at: stageData.created_at,
        updated_at: stageData.updated_at,
      });
      ...
      return newProductStage._id; // Return the new stage ID
    }

    // If it exists, return the existing stage ID
...
const insertLeads = async () => {
  try {
    const leadsData = loadJSON('updated_leads.json');
    const pipelinesData = loadJSON('pipelines.json');
    ...
    const labelsMap = labelsData ? new Map(labelsData.map(label => [label.id, label.name])) : null; // Check for null
    ...
    // Create a map of lead type IDs and data
    ...
    for (const lead of leadsData) {
      ...
      const labels = await Promise.all(lead.labels.map(async (labelId) => {
        const labelData = labelsData.find(label => label.id === labelId);
        if (labelData) {
          return await findOrCreateLabel(labelData);
        }
        return null;
      }));
      ...
      await newLead.save();
      console.log(`Lead ${lead.id} inserted successfully`);
    }
