...
router.get('/dashboard-status-users-by-pipeline', isAuth, async (req, res) => {
    try {
        const pipelineIds = req.user.pipeline.map(id => new ObjectId(id));
        ...
        // Function to get month-wise finance stats for a user
...
const getMonthWiseStats = async (userId, pipelineId) => {
    ...
    return monthWiseStats;
        ...
        // Fetch stats for each pipeline and its users, including finance stats
        const pipelineStats = await Promise.all(
            pipelines.map(async ({ _id: pipelineId, name: pipelineName }) => {
                // Fetch users associated with the pipeline, including target
                const users = await User.find({ pipeline: pipelineId }, { name: 1, target: 1 });
