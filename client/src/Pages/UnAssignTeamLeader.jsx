...
const TeamLeaderUnassigned = () => {
    ...
    const branchID = useSelector((state) => state.loginSlice.user?.branch[0]?._id);
    ...
    const [selectedProduct, setSelectedProduct] = useState(product[0]?._id || "");
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(
                `/api/users/get-users-by-branch/${branchID}/${selectedProduct}`
    ...
    const fetchUnassignedLeads = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `/api/leads/unassigned-leads-for-team-leader/${selectedProduct}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
