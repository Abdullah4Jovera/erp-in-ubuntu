...
const EditContract = ({ setLeadToContract, setContractModal, singleLead, leads, rtl, setEditModal, editModal, contract }) => {
    // Abu Dhabi Branch
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.put(`/api/contracts/update-service-commission/${contract._id}`, updatedData, {
                headers: {
        ...
        const fetchUsers = async () => {
            try {
                ...
                const leadResponse = await axios.get(
                    `/api/contracts/single-contract/${contract._id}`,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
            ...
            >
                ...
                <Modal.Body>
                    <Form>
                        ...
                        {/* Third Card */}
                        {contract.lead_type?.name === 'Others' && contract.source_id?.name === 'Third Party' && (
                        ...
                        <Card className='convertToLead_card mt-2 modal_body_bg_color' style={{ border: '#d7aa47' }} >
                            ...
                            {contract?.lead_type?.name !== "Others" && (
                                <Card className='convertToLead_card mt-2 input_field_input_field'>
                                    {contract && contract?.lead_type && (
                                        <>
                                            {contract?.lead_type?.name !== "Others" && (
                                                <h5 className="heading_tag mutual_class_color">
                                                    {`${contract?.lead_type?.name} Team`}
                                                </h5>
                                            )}
                                            <Row>
