...
const FileUploaderContract = ({ id, contract, fetchContract }) => {
    const token = useSelector(state => state.loginSlice.user?.token);
    const permissions = useSelector(state => state.loginSlice.user?.permissions)
    const { lead_id: { files } = {} } = contract;
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(
                `${process.env.REACT_APP_BASE_URL}/api/leads/delete-file/${contract.lead_id?._id}/${deleteFileID}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
    ...
    return (
        <>
            <Card className="shadow card_discussion_contract mt-2 mb-3 mutual_background_class" style={{ padding: '20px', borderRadius: '10px' }}>
                <div className='discussion_files'>
                    <h5 className='mutual_class_color'>Files</h5>
                    {
                        contract.is_reject && canAddFileLead
                            ? null
                            : (
                                canAddFileLead && (
                        ...
                        {files.map((file, index) => {
                            ...
                            return (
                                <tr key={index} style={{ height: '40px' }}>
                                    <td onClick={() => handleFileClick(file, fileType)} style={{ verticalAlign: 'middle', backgroundColor: '#2d3134' }}>
                                        {fileType === 'image' ? (
                                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                ...
                                                />
                                                <span className='mutual_class_color' style={{ fontWeight: '600', fontSize: '12px' }}>  {file.file_name.slice(0, 10)}</span>
                                            </div>
                                        ) : fileType === 'pdf' ? (
                                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                    ...
                                    {
                                        contract.is_reject && canRemoveFileLead
                                            ? null
                                            : (
                                                canRemoveFileLead && (
