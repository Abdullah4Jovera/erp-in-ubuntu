...
const RejectedContract = () => {
  ...
  const fetchRejectedLeads = async () => {
    try {
      const response = await axios.get(`/api/contracts/rejected-contracts`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      setRejectedLeads(response.data.contractDetails || []);
    } catch (error) {
  ...
  const translations = {
    en: {
      rejectedLeads: "Rejected Contract",
            ...
            <Card className='leads_main_cards mt-3' style={{ padding: '0px 20px 10px 20px' }}  >
              <div style={{ position: 'relative' }} >
                <h2 className="text-center mutual_heading_class mt-3">
                  {language.rejectedLeads} ({rejectedLeads.length} {rejectedLeads.length === 1 ? 'Lead' : 'Contract'})
                </h2>
              ...
              <Table striped bordered hover responsive className='mt-1 table_main_container' size='md' variant='dark'>
                <thead style={{ backgroundColor: '#d7aa47' }}>
                  ...
                  >
                    ...
                    <th style={{ backgroundColor: '#d7aa47', color: "white", textAlign: 'center' }}>{language.productName}</th>
                    <th style={{ backgroundColor: '#d7aa47', color: "white", textAlign: 'center' }}>{language.contractStage}</th>
                  ...
                  {currentLeads.length > 0 ? (
                    currentLeads.map((lead) => (
                      <tr key={lead.id} className='table_td_class'>
                        ...
                        <td className="cell-width table_td_class">{lead.productName}</td>
                        <td className="cell-width table_td_class">{lead.contractStage}</td>
                        <td className="cell-width table_td_class">{lead.branchName}</td>
                        <td className="cell-width table_td_class">
                          <Link to={`/contracts/${lead.id}`} >
                            <GrView style={{ color: '#ffa000', fontSize: '20px', cursor: 'pointer' }} />
                          </Link>
                          <FcCancel className="mx-2" onClick={() => handleShowReason(lead.rejectReason)} style={{ cursor: 'pointer', color: 'red', fontSize: '20px', }} />
                        </td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="6" className="text-center table_td_class">No Contract Found</td>
                    </tr>
                  )}
                </tbody>
              </Table>
              ...
              >
                ...
                >
                  <Modal.Title className="mutual_class_color">
                    {rtl === 'true' ? 'سبب رفض العميل' : 'Rejected Contract Reason'}
                  </Modal.Title>
                </Modal.Header>
...
export default RejectedContract;
